...
import modifiedJ48.Evaluation;
import weka.classifiers.Sourcable;
import weka.classifiers.evaluation.ThresholdCurve;
import modifiedJ48.BinC45ModelSelection;
import modifiedJ48.C45ModelSelection;
import modifiedJ48.C45PruneableClassifierTree;
import modifiedJ48.ClassifierTree;
import modifiedJ48.ModelSelection;
import modifiedJ48.PruneableClassifierTree;
...
TechnicalInformationHandler {
	...
	/** Unpruned tree? */
	private boolean m_unpruned = false; //false

	/** Confidence level */
	private float m_CF = 0.5f; //Update by Qutub default 0.25
   ...
   <!-- options-end -->
	...
	/*public static void roc(Instances data) throws Exception {
	...
	public static void main(String [] argv) throws Exception{
		...
		//filePath = filePath + "/src/dataSets/covtype.arff";
		filePath = filePath + "/src/dataSets/Soyabean.arff";
		...
		randData.stratify(10); //In case your data has a nominal class and you wanna perform stratified cross-validation:
		
		Classifier cls = new ModifiedJ48();
		Evaluation evalTrain = new Evaluation(randData);
		
		long startTime = System.currentTimeMillis();
		
		for (int f=0; f < 10; f++) {
			
			Instances train = randData.trainCV(10, f);
			Instances test = randData.testCV(10, f);
			System.out.println("Train Instances No: " + train.numInstances());
			System.out.println("Test Instances No: " + test.numInstances());
			
			cls.buildClassifier(train);
			evalTrain.evaluateModel(cls, test);
			
			int folds = f+1; 
			System.out.println("weightedAreaUnderROC: " + evalTrain.weightedAreaUnderROC());
			System.out.println(evalTrain.toSummaryString("Result for fold: " + folds, true));
			System.out.println(evalTrain.toClassDetailsString("=== Detailed Accuracy By Class ==="));
			//System.out.println(evalTrain.toMatrixString("toMatrixString"));
	...
	// build roc curve
	public static void getRoc(Evaluation eval) throws Exception {
		// generate curve
		  ThresholdCurve tc = new ThresholdCurve();
		  int classIndex = 0;
		  Instances result = tc.getCurve(eval.predictions(), classIndex);
